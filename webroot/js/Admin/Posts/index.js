...
const getOrder = () => {
    ...
    productOrderList.forEach(productOrder => {
        if (!productOrder.classList.contains('js-dropZone')) {
            ...
            productOrder.querySelector('input[name="order[]"]').setAttribute('value', index)
        }
    })
}
...
document.getElementById('b').addEventListener('click',() => {
    document.querySelectorAll('.js-productOrder').forEach((content) => {
        if (!content.classList.contains('js-dropZone')) {
            const index = [].slice.call(document.querySelectorAll('.js-productOrder')).indexOf(content)
            content.querySelector('input[name="order[]"]').setAttribute('value', index)
        }
    })
})